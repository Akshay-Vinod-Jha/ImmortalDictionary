...
const CheckHistory = (props) => {
  ...
  useEffect(() => {
    ...
    setData([...JSON.parse(localStorage.getItem("HistoryOfDicitionary"))]);
  }, []);
  return (
    <div className="w-screen h-screen overflow-scroll no-scrollbar">
      ...
      >
            ...
            return (
              ...
              >
                <h1 className="absolute right-2 top-2 z-50 w-full flex justify-end items-center">
                  <MdDelete
                    className="text-lg h-max cursor-pointer rounded-md md:text-xl lg:text-2xl bg-[#ac42f3] w-max py-1 px-4"
                    onClick={() => {
                      console.log("To Delete " + index);
                      let temp = JSON.parse(
                        localStorage.getItem("HistoryOfDicitionary")
                      );
                      temp.splice(index, 1);
                ...
                <h1 className="w-full border-b-2 border-b-[#ffffff76] pb-2 text-lg font-bold tracking-wider md:text-xl lg:text-2xl">
        ...
        {JSON.parse(localStorage.getItem("HistoryOfDicitionary")).length ===
          0 && (
          <div className="w-full px-1 md:px-4 font-importantFont capitalize rounded-md h-auto py-2 bg-[#953cd0]">
            ...
            <h1 className="w-full mt-2 text-lg py-2 px-4 font-semibold tracking-wide text-[#ffffffdd]">
              No History Found On Your LocalStorage (No History Yet)
            </h1>
          </div>
        )}
      </div>
